...
def _contract_to_dict(db: Session, c: Contract, index_map: dict, as_of: date,
    ...
    d = {
        ...
        "notes": c.notes,
        "agent_id": str(c.agent_id) if getattr(c, "agent_id", None) else None,
        "deposit_amount": getattr(c, "deposit_amount", 0) or 0,
        "deposit_currency": getattr(c, "deposit_currency", None) or "ARS",
        "deposit_status": getattr(c, "deposit_status", None) or "none",
        "deposit_returned_at": c.deposit_returned_at.isoformat() if getattr(c, "deposit_returned_at", None) else None,
        "deposit_notes": getattr(c, "deposit_notes", None),
