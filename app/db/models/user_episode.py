...
class ZoneStat(Base):
    """Aggregated zone statistics from search patterns."""

    __tablename__ = "zone_stats"

    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    zone_name: Mapped[str] = mapped_column(String(50), unique=True, index=True)
...
class SearchFailure(Base):
    """Dead-end search criteria combos — avoid suggesting."""
    ...
    last_failed_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow)
