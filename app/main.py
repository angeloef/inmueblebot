...
async def lifespan(app: FastAPI):
    ...
    # Auto-reset context so every deploy starts fresh
    try:
        import os
        reset_all = os.environ.get("RESET_ALL_ON_STARTUP", "").lower() in ("true", "1", "yes")
        if reset_all:
...
# Billing (MercadoPago suscripción + webhook + gating)
...
_api_compat.include_router(documents_router)
_api_compat.include_router(reports_router)
_api_compat.include_router(exports_router)
