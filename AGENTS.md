...
- **Phase 3a (schema):** added dedicated indexed `User.bsuid` column (`String(150)`, nullable, non-unique index `ix_users_bsuid`). Startup migration (`admin.py:_run_startup_migration`) adds the column + index. `get_by_bsuid` queries the column (`.first()`); `_capture_identity` and `schedule_visit` write the column. `whatsapp_phone` left `NOT NULL`/`UNIQUE`.
  - ⚠️ GOTCHA (fixed): the first attempt also ran a backfill `UPDATE ... extra_data ? 'bsuid'`. That FAILED because `users.extra_data` is TEXT (not JSONB) in some instances, and since the whole migration is ONE transaction, the failure rolled back the `ADD COLUMN bsuid` too → `column users.bsuid does not exist` on EVERY User query → scheduling/leads/delete all 500'd. Fix: removed the backfill; `_capture_identity` repopulates bsuid per user on their next message. Lesson: don't add a statement that can fail to the shared-transaction startup migration.
- **Phase 1 completion (context):** the scheduling field-gathering context in `context_aggregator.py` was STILL requiring `teléfono` (separate from the S2 system prompt fixed earlier) → the bot kept looping asking for the phone. Now the blockers are day+time only; the name comes from the LLM/conversation (validated by schedule_visit); teléfono is never requested; and the "ready" instruction tells the LLM to call schedule_visit and not confirm in text without it.
